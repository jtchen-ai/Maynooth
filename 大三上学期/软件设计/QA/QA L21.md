...
- 这个模式促进了松耦合。它减少了系统中的子类数量。
- 你可以把多对多的关系替换成一对多的关系，所以读起来和理解起来更容易。
...
如果把太多的逻辑放进去，中介者对象的架构可能会变得很复杂。不恰当地使用中介者模式可能会导致出现上帝类反模式。。
...
对象之间过多的互连会形成一个单体系统（对象之间的关系十分混乱），导致系统的行为很难改变，因为系统的行为分布在许多对象中。作为一个副作用，你可能需要创建许多子类才能在这个系统中引入那些改变。
