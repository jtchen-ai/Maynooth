...
回答：你可以实现一个类似于装饰器的保护代理，但你不应该忘记它们的意图。装饰器关注的是增加职责，而代理关注的是控制对对象的访问。代理之间因为类型和实现的不同而不同。另外，一般来说，代理是在相同的接口上工作，但装饰器可以在扩展的接口上工作。所以，如果你能记住它们的目的，在大多数情况下，你可以清楚地把它们和装饰器区分开来。

## 问题 8：代理有什么缺点？

回答：如果你在实现时足够小心，优点是远大于缺点的，但是：

你可以对响应时间提出担忧。因为你不是直接和实际对象对话，通过这些代理的响应时间可能会更长。
